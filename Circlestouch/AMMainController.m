...
@implementation AMMainController
...
- (void)viewDidLoad
{
    ...
    AMColorsPanel *cp = [[AMColorsPanel alloc] initWithFrame:CGRectMake(0,
                                                                        0,
                                                                        SCREEN_WIDTH,
                                                                        MARGIN_TOP)
    ...
    AMLivesRepresentation *lr = [[AMLivesRepresentation alloc] initWithFrame:CGRectMake(x,
...
- (void)circleTouched:(AMCircleButton *)sender
{
...
- (void)circleExpired:(AMCircleButton *)sender
{
    if (![self.gameManager isItOkToTouchColor:sender.color]) {
...
- (void)vanishRightCircle:(AMCircleButton *)sender
{
    ...
    [self.gameManager.circles performSelector:@selector(removeObject:)
                                   withObject:[[AMCircle alloc] initWithX:sender.frame.origin.x - BUTTON_SIZE / 2
                                                                     andY:sender.frame.origin.y - BUTTON_SIZE / 2
                                                                 andColor:nil]
                                   afterDelay:VANISH_DURATION_RIGHT];
}

- (void)vanishWrongCircle:(AMCircleButton *)sender
{
    [sender removeTarget:self action:@selector(circleTouched:) forControlEvents:UIControlEventTouchUpInside];
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(circleExpired:) object:sender];
...
- (void)createNewCircle
{
    ...
    // Creating AMCircleButton and adding to main view
    AMCircleButton *buttonCircle = [[AMCircleButton alloc] initWithFrame:CGRectMake(x, y, BUTTON_SIZE, BUTTON_SIZE) andColor:color];
    ...
    // Creating AMCircle and adding to GameManager
    AMCircle *circle = [[AMCircle alloc] initWithX:buttonCircle.frame.origin.x
    ...
    // Removing AMCircleButton (from main view) and AMCircle (from GameManager) after it expires
...
- (void)userPressedResumeOrNewGame:(id)sender
{
    ...
    if ([self gameStatus] == AMGameStatusGameOver) {
        self.gameManager = [[AMGameManager alloc] init];
        self.colorsPanel.colorsToTouch = self.gameManager.colorsToTouch;
        self.colorsPanel.colorsToAvoid = self.gameManager.colorsToAvoid;
        [self.timePlayingButton setTitle:[NSString stringWithFormat:@"%i", self.gameManager.timePlaying] forState:UIControlStateNormal];
...
- (AMGameStatus)gameStatus
{
    if (self.gameManager.livesRemaining > 0 &&
        ...
        self.gameManager.circlesTouchedWell == 0)
            return AMGameStatusBeforeFirstGame;
    
    if (self.gameManager.livesRemaining > 0)
        return AMGameStatusGamePaused;
    
    if (self.gameManager.livesRemaining == 0)
        return AMGameStatusGameOver;
    
    return AMGameStatusUndefined;
}
...
{
    if ([self gameStatus] == AMGameStatusGameOver) return;
...
- (void)decreaseLivesRemaining
{
    ...
    if ([self gameStatus] == AMGameStatusGameOver) {
        [self inactivateCircleCreationTimer];
        [self inactivateTimePlayingTimer];
        // Checking AMPageControlController isn't shown, as game can finish while paused
...
- (void)didReceiveMemoryWarning
{
    ...
    NSLog(@"AMMainController > didReceiveMemoryWarning");
}
